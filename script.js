// آزمایشگاه مجموعه‌های ریاضی - script.js - V.4.40
...
function detectInfinityType(expression, elements) {
    const l = expression.toLowerCase();
    
    if(l.includes('کمتر از') || l.includes('کوچکتر از') || l.includes('<')) {
        return 'left';
    }
    if(l.includes('بزرگتر از') || l.includes('بیشتر از') || l.includes('>')) {
        return 'right';
    }
    
    if(l.includes('صحیح') || l.includes('ℤ')) {
        if(!l.includes('بین') && !l.includes('تا')) {
            return 'both';
        }
    }
    
    if(l.includes('طبیعی') || l.includes('ℕ') || l.includes('حسابی') || l.includes('𝕎')) {
        return 'left';
    }
...
function showAllSets() { 
    ...
    AppState.sets.forEach((d,n)=>{const e=d.elements||[]; let c=''; let infType = d.infinityType || 'none';
        if(d.type==='symbolic'){const dis=d.isInfinite?formatInfiniteSet(e, infType)+'<br><small style="color:#856404;">⚠️ مجموعه نامتناهی است - نمی‌توان تمام اعضا را نوشت</small>':formatSet(e);c=`<div class="set-expression">${d.expression}</div><div class="set-content">مقادیر: ${dis}</div>`;} else if(d.type==='verbal'){const dis = (infType !== 'none') ? formatInfiniteSet(e, infType) : formatSet(e); c=`<div class="set-description">${d.description}</div><div class="set-content">${dis}</div>`;} else c=`<div class="set-content">${formatSet(e)}</div>`; h+=`<div class="set-item"><div class="set-name">${n} <small>(${getTypeName(d.type)})</small></div>${c}<div class="set-actions"><button onclick="editSet('${n}')" class="btn btn-info">✏️ ویرایش</button><button onclick="deleteSet('${n}')" class="btn btn-danger">🗑️ حذف</button><button onclick="showSetDetails('${n}')" class="btn btn-secondary">📊 جزئیات</button></div></div>`;}); 
...
function showLivePreview() { const e=document.getElementById('symbolicExpression').value.trim(),pc=document.getElementById('previewContainer'),lp=document.getElementById('livePreview'); if(!e){pc.style.display='none';return;} try{const el=parseSymbolicExpression(e);const isInf=hasUniversalSet(e)&&isOneSidedCondition(e);if(isInf){const cd=detectConditionDirection(e);const sn=getUniversalSetName(e);let displayEl=el;let infType = 'none'; if(cd.direction === '<' || cd.direction === '<=') infType = 'left'; else if(cd.direction === '>' || cd.direction === '>=') infType = 'right'; if(cd.threshold!==null&&sn&&(sn==='ℤ'||sn==='ℕ'||sn==='𝕎')){const generated=generateInfiniteSample(sn,cd.direction,cd.threshold);if(generated.length>0)displayEl=generated;} lp.innerHTML=`<p><strong>عناصر (نمونه):</strong> ${formatInfiniteSet(displayEl, infType)}</p><p style="color:#856404;background:#fff3cd;padding:10px;border-radius:8px;"><strong>⚠️ مجموعه نامتناهی است</strong> - نمی‌توان تمام اعضا را نوشت.</p>`;}else{lp.innerHTML=`<p><strong>عناصر:</strong> ${formatSet(el)}</p><p><strong>تعداد:</strong> ${el.length}</p><p><strong>نمایش:</strong> [${el.join(', ')}]</p>`;}pc.style.display='block';}catch(err){lp.innerHTML=`<p style="color:var(--danger-color);">${err.message}</p>`;pc.style.display='block';} }
function debounce(f,w){let t;return function(...a){clearTimeout(t);t=setTimeout(()=>f(...a),w);};}
function testSymbolicParser(){const e=document.getElementById('symbolicExpression').value.trim();if(!e){showMessage('عبارت را وارد کنید','warning');return;}try{const el=parseSymbolicExpression(e);const isInf=hasUniversalSet(e)&&isOneSidedCondition(e);if(isInf){const cd=detectConditionDirection(e);const sn=getUniversalSetName(e);let displayEl=el;let infType = 'none'; if(cd.direction === '<' || cd.direction === '<=') infType = 'left'; else if(cd.direction === '>' || cd.direction === '>=') infType = 'right'; if(cd.threshold!==null&&sn&&(sn==='ℤ'||sn==='ℕ'||sn==='𝕎')){const generated=generateInfiniteSample(sn,cd.direction,cd.threshold);if(generated.length>0)displayEl=generated;} showMessage(`⚠️ مجموعه نامتناهی - نمونه: ${formatInfiniteSet(displayEl, infType)}`,'warning',6000);}else{showMessage(`✅ ${formatSet(el)}`,'success',5000);}}catch(err){showMessage(`❌ ${err.message}`,'error',5000);}}
function createSymbolicSet(){const n=document.getElementById('setName').value.trim(),e=document.getElementById('symbolicExpression').value.trim();if(!n||!e){showMessage('فیلدها را پر کنید','error');return;}if(AppState.sets.has(n)){showMessage(`"${n}" تکراری`,'warning');return;}try{let el=parseSymbolicExpression(e);const isInf=hasUniversalSet(e)&&isOneSidedCondition(e);let infType = 'none'; if(isInf){const cd=detectConditionDirection(e);const sn=getUniversalSetName(e); if(cd.direction === '<' || cd.direction === '<=') infType = 'left'; else if(cd.direction === '>' || cd.direction === '>=') infType = 'right'; if(cd.threshold!==null&&sn&&(sn==='ℤ'||sn==='ℕ'||sn==='𝕎')){const generated=generateInfiniteSample(sn,cd.direction,cd.threshold);if(generated.length>0)el=generated;}} AppState.sets.set(n,{type:'symbolic',elements:el,expression:e,isInfinite:isInf,infinityType:infType,createdAt:new Date().toISOString()});HistoryManager.pushState(HistoryManager.getCurrentStateForHistory());StorageManager.saveState();updateStats();if(isInf){showMessage(`✅ "${n}" ایجاد شد (مجموعه نامتناهی): ${formatInfiniteSet(el, infType)}`,'warning',6000);}else{showMessage(`✅ "${n}" ایجاد شد: ${formatSet(el)}`,'success');}showAllSets();}catch(err){showMessage(`❌ ${err.message}`,'error');}}
...
function showSetDetails(n){const d=AppState.sets.get(n);if(!d)return;let iw='';if(AppState.infiniteSetsInfo&&AppState.infiniteSetsInfo[n])iw=`<p style="color:#856404;background:#fff3cd;padding:10px;border-radius:8px;margin:12px 0;">${AppState.infiniteSetsInfo[n]}</p>`;let elDisplay=(d.infinityType && d.infinityType !== 'none') ? formatInfiniteSet(d.elements, d.infinityType)+'<br><small style="color:#856404;">⚠️ مجموعه نامتناهی است - نمی‌توان تمام اعضا را نوشت</small>':formatSet(d.elements);document.getElementById("step").innerHTML=`<div class="step-container"><h3>📊 ${n}</h3><div class="set-details"><p><strong>نوع:</strong> ${getTypeName(d.type)}</p><p><strong>تعداد:</strong> ${d.elements.length}</p><p><strong>اعضا:</strong> ${elDisplay}</p>${d.expression?`<p><strong>نمادین:</strong> ${d.expression}</p>`:''}${d.description?`<p><strong>توصیف:</strong> ${d.description}</p>`:''}${d.operation?`<p><strong>عملیات:</strong> ${d.operation}</p>`:''}${iw}<p><strong>زمان:</strong> ${new Date(d.createdAt).toLocaleString('fa-IR')}</p></div><button onclick="showAllSets()" class="btn btn-secondary">🔙</button></div>`;}
...
// راه‌اندازی
document.addEventListener('DOMContentLoaded',function(){console.log('🚀 V.4.40');StorageManager.clearAllData();setTimeout(()=>{const l=document.getElementById('loading');if(l)l.style.display='none';},800);try{SetOperationsTests.runAllTests();}catch(e){}HistoryManager.pushState(HistoryManager.getCurrentStateForHistory());InputTracker.init();SmartKeyboard.init();document.getElementById('startBtn')?.addEventListener('click',startNew);document.getElementById('addSetBtn')?.addEventListener('click',addNewSet);document.getElementById('helpBtn')?.addEventListener('click',showHelp);document.getElementById('debugBtn')?.addEventListener('click',debugAppState);document.getElementById('mainMenuBtn')?.addEventListener('click',goToMainMenu);document.addEventListener('keydown',e=>{if(e.ctrlKey||e.metaKey){if(e.key==='z'){e.preventDefault();undoLastAction();}}});updateStats();showMainMenu();console.log('✅ V.4.40 آماده');});
document.addEventListener('scroll',function(){const b=document.getElementById('scrollToTop');if(b){if(window.pageYOffset>300)b.classList.add('show');else b.classList.remove('show');}});
